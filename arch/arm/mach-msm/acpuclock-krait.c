...
static u32 get_PTE_value(int Is_pte)
{
	void __iomem *qfprom_base;
	u32 pte_efuse = 0;

	if (Is_pte)
		qfprom_base = ioremap(0x007040B8, SZ_256);
	else
		qfprom_base = ioremap(0x007040BC, SZ_256);	
...
static int __init get_pvs_bin(u32 pte_efuse)
{
	...
	if(cpu_is_apq8064())
	{
		...
		PTE_Lsb = get_PTE_value(1);
		dev_info(drv.dev, "PTE LSB is 0x%x \n",PTE_Lsb);			

		PTE_Msb = get_PTE_value(0);
		dev_info(drv.dev, "PTE MSB is 0x%x \n",PTE_Msb);			
