...
#if defined(CONFIG_TOUCHSCREEN_CYTTSP_TOUCHKEY)
#if (CONFIG_BOARD_VER < CONFIG_WS10)
#define GPIO_TOUCHKEY_SCL  28
#else
#define GPIO_TOUCHKEY_SCL  62
#endif
#define GPIO_TOUCHKEY_SDA 6
#define TOUCHKEY_I2C_BUS_ID    18
#define TOUCHKEY_I2C_CHIP_ADDR 0x20
...
struct platform_device msm_device_i2c_gpio_touchkey = {
	.name = "i2c-gpio",
	.id = TOUCHKEY_I2C_BUS_ID, // id important value
	.dev = {
		.platform_data = &i2c_gpio_touchkey_data,
	}
};

static struct i2c_board_info i2c_touchkey_devices[]  = {
	{
		I2C_BOARD_INFO("cypress_touchkey", TOUCHKEY_I2C_CHIP_ADDR),
	},
};

static uint32_t touch_i2c_gpio_table[] = {
	GPIO_CFG(GPIO_TOUCHKEY_SCL, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(GPIO_TOUCHKEY_SDA, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};

static void sky_touchkey_i2c_gpio_init(void)
{
	int ret = 0, i=0;
	for (i = 0; i < ARRAY_SIZE(touch_i2c_gpio_table); ++i) {
		ret= gpio_tlmm_config(touch_i2c_gpio_table[i], GPIO_CFG_ENABLE);
		if ( ret ) {
			pr_err( "%s: Failed touch_i2c_gpio_table i2c gpio_tlmm_config(%d)=%d\n", __func__, i, ret);
			break;
		}
	}

	i2c_register_board_info(TOUCHKEY_I2C_BUS_ID, i2c_touchkey_devices, ARRAY_SIZE(i2c_touchkey_devices));
}
...
#define GPIO_SC_SCL      57    
#define GPIO_SC_SDA      56
#else
#define GPIO_SC_SCL      69    
#define GPIO_SC_SDA      26
#endif 
...
#ifdef CONFIG_ANDROID_PANTECH_USB_PHY_PARAMETER
...
#if defined(CONFIG_PIEZO_ECO) || defined(CONFIG_INPUT_IR_SENSOR)
	&apq8064_device_qup_i2c_gsbi5,
#endif	// CONFIG_PIEZO_ECO || CONFIG_INPUT_IR_SENSOR
};
...
static struct platform_device *early_common_devices[] __initdata = {
	...
	&apq8064_device_dmov,
#if !defined(CONFIG_INPUT_IR_SENSOR)
	&apq8064_device_qup_spi_gsbi5,
#endif	// !CONFIG_INPUT_IR_SENSOR
#ifdef CONFIG_SKY_DMB_SPI_HW
...
#if defined(CONFIG_INPUT_IR_SENSOR)
#define IR_I2C_NAME   		 "irmotion"
#define IR_I2C_ADDR			 0x73

#define ALS_I2C_NAME   		 "pac7672"	// Ambient Light Sensor
#define ALS_I2C_ADDR			 0x90

#define IR_18V_EN_GPIO			34

static struct msm_i2c_platform_data apq8064_i2c_qup_gsbi5_pdata = {	// for APQ8064
	.clk_freq = 384000,
	.src_clk_rate = 24000000,
    .use_gsbi_shared_mode = 1,	// to share APQ_UART_TX/RX 
};

static struct i2c_board_info pac7620_i2c_info[] = {
	{
		I2C_BOARD_INFO(IR_I2C_NAME, IR_I2C_ADDR),
	},
	{
		I2C_BOARD_INFO(ALS_I2C_NAME, ALS_I2C_ADDR >> 1),	// 7bit address
	},
};

#if (CONFIG_BOARD_VER > CONFIG_PT10)
static uint32_t gesture_i2c_gpio_table[] = {
	GPIO_CFG(IR_18V_EN_GPIO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
};

static void enable_als_28v_ldo(void)
{
	int als_en_gpio = PM8921_GPIO_PM_TO_SYS(13);	// ALS_28V_EN, 164
	int rc;

	rc = gpio_request(als_en_gpio, "als_28v_en");
	if (rc)
		pr_err("request for als_28v_en gpio failed,"
						 "rc=%d\n", rc);
	else
		gpio_set_value_cansleep(als_en_gpio, 1);
}

#else	// CONFIG_PT10

#define IR_LEVEL_SHIFT_EN_GPIO	33

static uint32_t gesture_i2c_gpio_table[] = {
	GPIO_CFG(IR_18V_EN_GPIO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
	GPIO_CFG(IR_LEVEL_SHIFT_EN_GPIO, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_UP, GPIO_CFG_2MA),
};
#endif	// CONFIG_BOARD_VER > CONFIG_PT10

static void pantech_gesture_gpio_init(void)
{
	int rc = 0, i;
	
	for (i = 0; i < ARRAY_SIZE(gesture_i2c_gpio_table); ++i) {
		rc = gpio_tlmm_config(gesture_i2c_gpio_table[i], GPIO_CFG_ENABLE);
		if ( rc ) {
			pr_err( "%s: Failed gesture gpio_tlmm_config(%d)=%d\n", __func__, i, rc);
			break;
		}
	}

	rc = gpio_request(IR_18V_EN_GPIO,"ir_18v_enable");
	rc = gpio_direction_output(IR_18V_EN_GPIO,1);
	gpio_set_value(IR_18V_EN_GPIO, 1);	
	rc = gpio_export(IR_18V_EN_GPIO,true);
	printk("%s : IR_18V_EN_GPIO Setting (%d)\n",__func__,rc);

#if (CONFIG_BOARD_VER == CONFIG_PT10)
	rc = gpio_request(IR_LEVEL_SHIFT_EN_GPIO,"ir_level_shift_enable");
	rc = gpio_direction_output(IR_LEVEL_SHIFT_EN_GPIO,1);
	gpio_set_value(IR_LEVEL_SHIFT_EN_GPIO, 1);
	rc = gpio_export(IR_LEVEL_SHIFT_EN_GPIO,true);
	printk("%s : IR_LEVEL_SHIFT_EN_GPIO Setting (%d)\n",__func__,rc);
#endif  // CONFIG_BOARD_VER == CONFIG_PT10
}
#endif	// CONFIG_INPUT_IR_SENSOR
...
#elif defined(CONFIG_MACH_APQ8064_EF52W) && (CONFIG_BOARD_VER <= CONFIG_PT10)
#define GPIO_KEY_VOLUME_UP		PM8921_GPIO_PM_TO_SYS(9)
#define GPIO_KEY_VOLUME_DOWN_PM8921	PM8921_GPIO_PM_TO_SYS(10)
#define GPIO_KEY_HOME				PM8921_GPIO_PM_TO_SYS(2)
#elif  defined(CONFIG_MACH_APQ8064_EF52W) && (CONFIG_BOARD_VER >= CONFIG_WS10)
#define GPIO_KEY_VOLUME_UP		PM8921_GPIO_PM_TO_SYS(1)
#define GPIO_KEY_VOLUME_DOWN_PM8921	PM8921_GPIO_PM_TO_SYS(9)
#define GPIO_KEY_HOME				PM8921_GPIO_PM_TO_SYS(10)
#define GPIO_KEY_AQUA				PM8921_GPIO_PM_TO_SYS(2)
#else
...
static struct i2c_registry apq8064_i2c_devices[] __initdata = {
...
#if defined(CONFIG_INPUT_IR_SENSOR)
	{
		I2C_SURF | I2C_FFA,
		APQ_8064_GSBI5_QUP_I2C_BUS_ID,
		pac7620_i2c_info,				// ir + als
		ARRAY_SIZE(pac7620_i2c_info),
	},
#endif	// CONFIG_INPUT_IR_SENSOR
	{
		I2C_MPQ_CDP,
...
#if defined(CONFIG_MACH_APQ8064_EF48S) ||defined(CONFIG_MACH_APQ8064_EF49K) || defined(CONFIG_MACH_APQ8064_EF50L) || defined(CONFIG_MACH_APQ8064_EF51S) || defined(CONFIG_MACH_APQ8064_EF51K) || defined(CONFIG_MACH_APQ8064_EF51L) || defined(CONFIG_MACH_APQ8064_EF52S) || defined(CONFIG_MACH_APQ8064_EF52K) || defined(CONFIG_MACH_APQ8064_EF52L) || defined(CONFIG_MACH_APQ8064_EF52W)
		 || (machine_is_apq8064_ef48s() || machine_is_apq8064_ef49k() || machine_is_apq8064_ef50l() || machine_is_apq8064_ef51s() || machine_is_apq8064_ef51k() || machine_is_apq8064_ef51l() || machine_is_apq8064_ef52s() ||  machine_is_apq8064_ef52k() ||  machine_is_apq8064_ef52l() || machine_is_apq8064_ef52w() || machine_is_apq8064_ef52w())
#endif
	  )
		mach_mask = I2C_FFA;
...
static void __init apq8064_common_init(void)
{
  ...
  apq8064_device_qup_spi_gsbi7.dev.platform_data = &apq8064_qup_spi_gsbi7_pdata;
	...
	if (machine_is_apq8064_mtp()
#if defined(CONFIG_MACH_APQ8064_EF48S) ||defined(CONFIG_MACH_APQ8064_EF49K) || defined(CONFIG_MACH_APQ8064_EF50L) || defined(CONFIG_MACH_APQ8064_EF51S) || defined(CONFIG_MACH_APQ8064_EF51K) || defined(CONFIG_MACH_APQ8064_EF51L) || defined(CONFIG_MACH_APQ8064_EF52S) || defined(CONFIG_MACH_APQ8064_EF52K) || defined(CONFIG_MACH_APQ8064_EF52L) || defined(CONFIG_MACH_APQ8064_EF52W)
	  || (machine_is_apq8064_ef48s() || machine_is_apq8064_ef49k() || machine_is_apq8064_ef50l() || machine_is_apq8064_ef51s() || machine_is_apq8064_ef51k() || machine_is_apq8064_ef51l() || machine_is_apq8064_ef52s() ||  machine_is_apq8064_ef52k() ||  machine_is_apq8064_ef52l() || machine_is_apq8064_ef52w())
#endif
	) {
...
#if defined(CONFIG_INPUT_IR_SENSOR)
#if (CONFIG_BOARD_VER > CONFIG_PT10)
	// after pmic_gpio_mpp_init, ALS_2.8V_EN control
	enable_als_28v_ldo();
#endif
#endif	// CONFIG_INPUT_IR_SENSOR
...
MACHINE_START(APQ8064_EF52W, "PANTECH APQ8064 EF52W")
        .map_io = apq8064_map_io,
        .reserve = apq8064_reserve,
        .init_irq = apq8064_init_irq,
        .handle_irq = gic_handle_irq,
