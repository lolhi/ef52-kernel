...
#if defined(CONFIG_FEATURE_PANTECH_RESET_DSPS_CORE) //P12911 	2013.3.12	 add for reset the dsps core(+) FEATURE_PANTECH_RESET_DSPS_CORE
#define PANTECH_DSPS_RESET_ENABLE 1
#define PANTECH_DSPS_RESET_DISABLE 0
#define PANTECH_SUB_SYSTEM_DEFAULT	0xfffff
#define PANTECH_TEST_EANBLE		1
#define PANTECH_TEST_DISABLE		0x0
#endif
/**
 *  Driver Context
 *
...
struct dsps_drv {
	...
	atomic_t dsps_reset_flag;	
	atomic_t sub_system_restart;
	atomic_t pantech_test_enable;	
#endif
...
#if defined(CONFIG_FEATURE_PANTECH_RESET_DSPS_CORE) 
static ssize_t dsps_reset_show(struct device *dev, struct device_attribute *attr,
		char *buf)
{
	struct dsps_drv *sdev = (struct dsps_drv *)
		dev_get_drvdata(dev);
	int aflag=atomic_read(&sdev->dsps_reset_flag);
	printk("---call dsps_reset_show : [%d]\n",atomic_read(&sdev->dsps_reset_flag));	
	return sprintf(buf, "%d\n", aflag);
}
static ssize_t dsps_reset_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct dsps_drv *sdev = (struct dsps_drv *)
		dev_get_drvdata(dev);	
	int aflag=atomic_read(&sdev->dsps_reset_flag);
	int restart=atomic_read(&sdev->sub_system_restart);
	atomic_set(&sdev->pantech_test_enable,PANTECH_TEST_EANBLE);					
	if(restart==PANTECH_SUB_SYSTEM_DEFAULT) //store the ORIGNAL ssr value
		atomic_set(&sdev->sub_system_restart,get_restart_level());
	if(!aflag)
		atomic_set(&sdev->dsps_reset_flag,PANTECH_DSPS_RESET_ENABLE);
	printk("+++call DSPS RESET : [%d]\n",atomic_read(&sdev->dsps_reset_flag));
	return count;
}
int dsps_reset_state(void)
{
	int aflag=atomic_read(&drv->dsps_reset_flag);
	return aflag;
}
EXPORT_SYMBOL(dsps_reset_state);
static DEVICE_ATTR(dsps_reset, S_IRUGO | S_IWUSR,dsps_reset_show, dsps_reset_store);
#endif
...
#if defined(CONFIG_FEATURE_PANTECH_RESET_DSPS_CORE) 
	int aflag=0;
	int sub_restart_level=0;
#endif	
	pr_debug("%s.\n", __func__);
	drv->pil = pil_get(name);
#if defined(CONFIG_FEATURE_PANTECH_RESET_DSPS_CORE) 
	aflag=atomic_read(&drv->pantech_test_enable);		
	if(aflag==PANTECH_TEST_EANBLE){
		dump_stack();		
		aflag=atomic_read(&drv->dsps_reset_flag);	
		if(aflag){
		atomic_set(&drv->dsps_reset_flag, PANTECH_DSPS_RESET_DISABLE);		
		sub_restart_level=atomic_read(&drv->sub_system_restart);	
		restore_restart_level(sub_restart_level);
		}
	}
#endif	
	if (IS_ERR(drv->pil)) {
...
static void dsps_restart_handler(void)
{
	...
	if (atomic_add_return(1, &drv->crash_in_progress) > 1) {
		...
		if(aflag==PANTECH_TEST_EANBLE){
			dump_stack();			
			if (atomic_add_return(1, &drv->dsps_reset_flag) > 2) {
				pr_err("%s: DSPS already resetting. dsps_reset_flag %d\n", __func__,
			       atomic_read(&drv->dsps_reset_flag));
			}
			else
...
#if defined(CONFIG_FEATURE_PANTECH_RESET_DSPS_CORE) 
	atomic_set(&drv->dsps_reset_flag,PANTECH_DSPS_RESET_DISABLE);	
	atomic_set(&drv->sub_system_restart,PANTECH_SUB_SYSTEM_DEFAULT);		
	atomic_set(&drv->pantech_test_enable,PANTECH_TEST_DISABLE);						
	ret = device_create_file(drv->dev, &dev_attr_dsps_reset);
	if (ret < 0)	
...
static int __devexit dsps_remove(struct platform_device *pdev)
{
	pr_debug("%s.\n", __func__);
#if defined(CONFIG_FEATURE_PANTECH_RESET_DSPS_CORE) 
	device_remove_file(drv->dev , &dev_attr_dsps_reset);
#endif
