...
#define NUM_PVS (7)
const char *pvs_info[NUM_SPEED_BINS][NUM_PVS] = {
	[0][0] = "1.5G_SLOW",
	[0][1] = "1.5G_NOMINAL",
    [0][2] = "1.5G_FAST",
    [0][3] = "1.5G_FASTER",
...
//2013.01.24 8064_1.7G thermal added by wooyub
...
#ifdef CONFIG_PANTECH_SOC_HW_REVISION_NUMBER
uint32_t socinfo_get_soc_hw_revision_number(void)
{
	uint32_t soc_hw_rev_num;
	/* APQ8064 SOC HW REVISION NUMBER */
	soc_hw_rev_num = readl((MSM_TLMM_BASE + 0x2054));
	return soc_hw_rev_num;
}
#endif
...
#ifdef CONFIG_PANTECH_SOC_HW_REVISION_NUMBER
static ssize_t
socinfo_show_hw_revision_number(struct sys_device *dev,
			 struct sysdev_attribute *attr,
			 char *buf)
{
	return snprintf(buf, PAGE_SIZE, "0x%x\n", socinfo_get_soc_hw_revision_number());
}

static struct sysdev_attribute socinfo_hw_revision_number_files[] = {
	_SYSDEV_ATTR(soc_hw_rev, 0444,
			socinfo_show_hw_revision_number, NULL),
};
#endif
...
static int get_speed_bin(u32 pte_efuse)
{
	...
	if (speed_bin == 0xF) {
		speed_bin = 0;
		printk(KERN_INFO "socinfo_show_pvs_info =>SPEED BIN: Defaulting to %d\n", speed_bin);
	} else {
		printk(KERN_INFO "socinfo_show_pvs_info =>SPEED BIN: %d\n", speed_bin);
	}
...
static ssize_t
socinfo_show_pvs_info(struct sys_device *dev,
		struct sysdev_attribute *attr,
		char *buf)
{
	uint32_t pte_efuse;
	...
	return snprintf(buf, PAGE_SIZE, "%-.32s\n",pvs_info[get_speed_bin(pte_efuse)][get_pvs_bin(pte_efuse)]);
}

static struct sysdev_attribute socinfo_pvs_info_files[] = {
	_SYSDEV_ATTR(pvs_info, 0444,
			socinfo_show_pvs_info, NULL),
};
//2013.01.15 p11219  <==
...
static int __init socinfo_init_sysdev(void)
{
    ...
    //2013.01.15 p11219
    socinfo_create_files(&soc_sys_device, socinfo_pvs_info_files,
            ARRAY_SIZE(socinfo_pvs_info_files));	
#ifdef CONFIG_PANTECH_SOC_HW_REVISION_NUMBER
	socinfo_create_files(&soc_sys_device, socinfo_hw_revision_number_files,
				ARRAY_SIZE(socinfo_hw_revision_number_files));
#endif	
...
#ifdef CONFIG_PANTECH_SOC_HW_REVISION_NUMBER
	pr_info("%s: SoC HW Revision NUM=0x%x\n", __func__, socinfo_get_soc_hw_revision_number());
#endif
