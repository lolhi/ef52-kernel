	...
	if( ulFreq == 196736 ) {
		...
		if( m_ucPLL_Mode == INPUT_CLOCK_24576KHZ) {
			if(INC_CMD_READ(ucI2CID, APB_GEN_BASE+ 0x01) != 0x0001) {
				...
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x03, 0x0000);	 
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x04, 0x0000);
				INC_CMD_WRITE(ucI2CID, APB_GEN_BASE+0x10, 0x0122);
			}
		}
		else if(m_ucPLL_Mode == INPUT_CLOCK_19200KHZ) {
...
INC_UINT8 INC_SYNCDETECTOR(INC_UINT8 ucI2CID, INC_UINT32 ulFreq, INC_UINT8 ucScanMode)
{
	...
	INC_UINT16	uiRefSleep = 20; //uiTimeOutCnt = 0, 
	//INC_UINT8	IsSyncNull = 0;
	ST_BBPINFO* pInfo;
	INC_UINT16	awData[2], uiTimeOut = 0; //uiRfNullTime = 240, 
	pInfo = INC_GET_STRINFO(ucI2CID);
	...
	while(1) {
		...
		awData[0] = INC_CMD_READ(ucI2CID, APB_RF_BASE+ 0xD1) & 0x3F; 
		awData[1] = (INC_CMD_READ(ucI2CID, APB_PHY_BASE+ 0x2C)>>10)&0x3F; 

      TDMB_MSG_INC_BB("State %d [0x%X][0x%X]\n",  wOperState, awData[0] , awData[1] );

		if(!wIsNullSync) {
			if (awData[1] < 0x1 || awData[1] > 0x10 ) {
			  TDMB_MSG_INC_BB("[%s] ERROR_SYNC_NO_SIGNAL1\n", __func__);
				pInfo->nBbpStatus = ERROR_SYNC_NO_SIGNAL;
				break;
			}
			if(awData[0] >= 0x39 || awData[0]  <= 0x20) {
		...
		if ((wIsNullSync == 1) && uiTimeOut >= (1200 / uiRefSleep)){
			if (awData[0] == 0) {
