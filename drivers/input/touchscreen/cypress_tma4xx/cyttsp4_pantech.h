...
static int _cyttsp4_get_small_finger_mode(struct cyttsp4 *ts, uint8_t mode) 
{
    int retval;
...
static int cyttsp4_small_finger_mode(struct cyttsp4 *ts, uint8_t enable)
{
    ...
    if(ts->small_finger_mode == enable) {
#ifdef CONFIG_TOUCHSCREEN_DEBUG
        if (ts->bus_ops->tsdebug >= CY_DBG_LVL_1)
            dev_info(ts->dev, "%s: small finger mode already %s\n", __func__, enable ? "enabled" : "disabled");
#endif /* --CONFIG_TOUCHSCREEN_DEBUG */
        goto exit_fingermode;
    }
    ...
    ts->small_finger_mode = _cyttsp4_get_small_finger_mode(ts, enable);
