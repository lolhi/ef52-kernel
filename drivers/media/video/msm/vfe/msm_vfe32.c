...
#if 0 //psj_test
#undef CDBG
#define CDBG(fmt, args...) printk(KERN_INFO "msm_vfe32 : " fmt, ##args)//pr_debug(fmt, ##args)
#endif
...
//pr_err("[SD_check] %s/ vfe32_ctrl->share_ctrl->dual_enabled : %d //<D7>",__func__,vfe32_ctrl->share_ctrl->dual_enabled);
	...
	struct axi_ctrl_t *axi_ctrl, struct msm_camera_vfe_params_t vfe_params)
{
	...
	if (axi_ctrl->share_ctrl->current_mode & VFE_OUTPUTS_RDI0) {
		...
		msm_camera_io_w((
				...
				axi_ctrl->share_ctrl->vfebase + VFE_BUS_CMD);
#if 1//def F_PANTECH_CAMERA_QPATCH_JPEG_ZSL //frame_base //for QCT_metadata_crash_check //META data test 
	...
	if (axi_ctrl->share_ctrl->current_mode & VFE_OUTPUTS_RDI2) {
		...
		msm_camera_io_w((
				...
				axi_ctrl->share_ctrl->vfebase + VFE_BUS_CMD);
		msm_camera_io_w(0x3,/*0x1,*/ axi_ctrl->share_ctrl->vfebase +
			vfe32_AXI_WM_CFG[axi_ctrl->share_ctrl->
			outpath.out4.ch0]);
	}
#endif
