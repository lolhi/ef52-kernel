...
// patch, fix bug in partial read on data buffer, by LEE, Younhee 2013.01.02
#define FEATURE_PANTECH_FIXBUG_PARTIAL_READ
...
// patch for fatal issue when protocol error occurs, 2013.03.21, Lee, Younhee
//	if (urb->status == -ENOENT && urb->actual_length > 0)
    if ((urb->status == -ENOENT || urb->status == -EPROTO) && 
        urb->actual_length > 0) 
		goto add_to_list;
