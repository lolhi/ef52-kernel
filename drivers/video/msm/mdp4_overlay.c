...
#if 0
		pr_info("*** %s: info->ptype=%d, info->pnum=%d, info->pndx=%d, info->mixer_num=%d\n"
...
void mdp4_mixer_stage_commit(int mixer)
{
	...
	if (data) // lived 2013.04.04 add patch from codeaurora
...
void mdp4_overlay_borderfill_stage_up(struct mdp4_overlay_pipe *pipe)
{
	...
	if (ctrl->baselayer[mixer]) {
		if (ctrl->baselayer[mixer]->pipe_type == OVERLAY_TYPE_BF)
			return;
	}
#else
	if (ctrl->baselayer[mixer])
...
#ifndef CONFIG_PANTECH_LCD_FIX_OTG_BLENDING_ISSUE	
...
#if 1//shkwak 20130206 qcom patch
			s_pipe->alpha_enable &&
#endif
		...
		if (s_pipe->is_fg) {
			...
			if (!alpha_drop) {
				...
				if ((!(s_pipe->flags & MDP_BLEND_FG_PREMULT)) &&
					...
					blend->fg_alpha = 0xff;
#ifdef PANTECH_LCD_QCPATCH_MDP_COMP_FIRST
				blend->op |= MDP4_BLEND_BG_INV_ALPHA; // QC_0131
#endif
			} else
				blend->op = MDP4_BLEND_BG_ALPHA_FG_CONST;
#ifndef PANTECH_LCD_QCPATCH_MDP_COMP_FIRST
			      ...
			      struct mdp4_overlay_pipe *plist)
{
	...
	for (i = 0; i < OVERLAY_PIPE_MAX; i++, pipe++) {
		...
		if (!(pipe->flags & MDP_BACKEND_COMPOSITION) && pipe->pipe_type == OVERLAY_TYPE_RGB) {
			fb_layer++;
		} else if (pipe->pipe_type == OVERLAY_TYPE_BF) {
			fb_layer++;
		}
#endif
