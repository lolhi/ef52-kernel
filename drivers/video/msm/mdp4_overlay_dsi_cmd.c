...
int get_wfd_info(void)
{
	pr_debug("[LCD]%s+++++: get %d\n", __func__, wfd_on);
	return wfd_on;
}

void set_wfd_info(bool value)
{
	pr_err("[LCD]%s+++++: set to %d, from %ps\n", __func__, value, __builtin_return_address(0) );
	wfd_on = value;
}
#endif // PANTECH_LCD_WFD_CONNECTION_FAIL_ON_CMDMODE
